...
from utils import embedding_device, get_model_path
...
class ThreadSafeObject:
    ...
    def acquire(self, owner: str = "", msg: str = ""):
        ...
        try:
            ...
            if log_verbose:
                logger.info(f"{owner} start opt: {self.key}。{msg}")
            yield self._obj
        finally:
            if log_verbose:
                logger.info(f"{owner} end opt: {self.key}。{msg}")
            self._lock.release()
...
class CachePool:
    ...
    def acquire(self, key: Union[str, Tuple], owner: str = "", msg: str = ""):
        cache = self.get(key)
        if cache is None:
            raise RuntimeError(f"resource key {key} not exists")
    ...
    def load_kb_embeddings(
        ...
        from db.repository.knowledge_base_repository import get_kb_detail
        ...
        if not self.get(key):
            ...
            with item.acquire(msg="initialize"):
                ...
                if model == "text-embedding-ada-002":  # openai text-embedding-ada-002
                    ...
                    if 'zh' in model:
                        # for chinese model
                        query_instruction = "generate searching docs"
                    elif 'en' in model:
                        # for english model
