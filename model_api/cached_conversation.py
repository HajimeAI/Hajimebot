...
class CachedConversation():
    def __init__(self, model: ChatModels = None):
        ...
        # )
        self._verbose = True
        self._stop = ['<|eot_id|>'] if LOCAL_LLM_MODEL.startswith('llama3') else None
    ...
    def simple_chat(self, query: str, memory=None):
        ...
        return conversation.predict(query=query, stop=self._stop)
    ...
    def kb_chat(self, query: str, reference: str):
        ...
        answer = conversation.predict(query=query, reference=reference, stop=self._stop)
        if answer != 'NOT_FOUND':
            return answer
